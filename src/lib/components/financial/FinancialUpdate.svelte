<script lang="ts">
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            dispatch("update", { reportId: report.id, data: reportData });
        } catch (err: any) {
...
<div class="bg-white/10 backdrop-blur-sm rounded-lg p-6">
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-6">
        <!-- Sélection de l'entreprise (readonly pour les rapports existants) -->
        <div>
            <label
                for="company"
            ...
            <input
                type="text"
                id="company"
                value={report?.company?.name || "Entreprise inconnue"}
