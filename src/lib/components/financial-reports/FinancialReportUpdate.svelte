<script lang="ts">
    ...
    onMount(() => {
        if (report) {
            formData = {
                company: report.company?.id || report.company || "",
                period_month: report.period_month || 1,
                period_year: report.period_year || 2000,
                ...
                employee_costs: report.employee_costs || 0,
                operational_costs: report.operational_costs || 0,
                marketing_costs: report.marketing_costs || 0,
                rd_costs: report.rd_costs || 0,
            };
        }
    });
    ...
    function handleSubmit() {
        ...
        dispatch("update", { reportId: report.id, data: { ...formData } });
    }
...
{#if report}
    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-6">
        <h2 class="text-2xl font-bold mb-6">Modifier le Rapport Financier</h2>
        ...
        <form on:submit|preventDefault={handleSubmit} class="space-y-6">
            ...
            <div class="flex gap-4">
                <button
                    type="submit"
                    disabled={submitting}
                    class="flex-1 bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-500 text-white font-semibold py-3 px-6 rounded-lg transition duration-300"
                >
                    {submitting ? "Modification..." : "Modifier le rapport"}
                </button>
                <button
                    type="button"
                    on:click={handleCancel}
                    class="flex-1 bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded transition duration-300"
                >
                    Annuler
                </button>
            </div>
