  ...
  async function handleAuth() {
    ...
    try {
      ...
      if (
        ...
        const validationError = err.data[firstKey];

        if (validationError?.message) {
          error = `${firstKey}: ${validationError.message}`;
        } else if (validationError && typeof validationError === "object") {
          const subKeys = Object.keys(validationError);
          if (subKeys.length > 0) {
            const subKey = subKeys[0];
            const subError = validationError[subKey];
            if (subError?.message) {
              error = `${subKey}: ${subError.message}`;
            }
          }
        }
      }
...
>
  ...
  >
    <div class="text-center mb-8">
      <h1 class="text-2xl md:text-3xl font-bold text-content-primary mb-2">
